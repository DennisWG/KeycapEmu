...
keycap::shared::database::database& get_kb_database()
{
    static keycap::shared::database::database kb_database{get_db_service()};
    return kb_database;
}

void init_databases(std::vector<std::thread>& thread_pool, config const& config)
{
    get_kb_database().connect(config.database.host, config.database.port, config.database.user,
...
int main()
{
    ...
    utility::set_console_title("KnowledgeBase");

    auto config = parse_config("kb.json");
    logging::create_loggers(config.logging);
    QUICK_SCOPE_EXIT(sc, [] { spdlog::drop_all(); });

    auto console = utility::get_safe_logger("console");
    console->info("Running KeycapEmu.KnowledgeBase version {}/{}", keycap::emu::version::GIT_BRANCH,
    ...
    keycap::knowledge_base::kb_service service;
    bool running = true;
